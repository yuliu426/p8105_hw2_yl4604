...
There are total `r nrow(distinct(nyc_transit_df, line))*nrow(distinct(nyc_transit_df, station_name))` distinct stations.
...
There are total `r nrow(distinct(nyc_df_ada, line))*nrow(distinct(nyc_df_ada, station_name))` stations are ADA compliant
...
```
There are `r round(nrow(nyc_df_novending)/nrow(nyc_transit_df)*100, 2)`% station entrances/exits without vending allow entrance.
...
There are total `r nrow(distinct(nyc_a_df, line))*nrow(distinct(nyc_a_df, station_name))` distinct stations serve the A train.
Of the stations that serves the A train, `r nrow(nyc_a_ada)` are ADA compliant.
